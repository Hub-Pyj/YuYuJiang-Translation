...
async function run(args) {
  ...
  const opts = Object.assign({
    ...
    batchSize: 20,       // 每次请求的字符串数
    gapMs: 60,           // 批次之间让出时间
    defaultMode: "frosted"   // 毛玻璃UI  "liquid"液态
  }, args || {});
...
${JSON.stringify({ items })}`;
  ...
  // 段控件滑动指示块：跟随选中项平滑滑动
  // 注意：用 offsetLeft/offsetWidth 布局坐标 不受父级scale动画影响
  ...
  injectStyle();
  buildUI();
  setMode(opts.defaultMode);   // 初始化：套用默认档预设
